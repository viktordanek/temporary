${MKDIR} --parents /build/observed/temporary &&
  ${FIND} /build/temporary.* -mindepth 0 -maxdepth 0 -type d -exec ${WC} > /build/observed/temporary/count.pre &&
  ...
  ${FIND} /build/temporary.* -mindepth 0 -maxdepth 0 -type d -exec ${WC} > /build/observed/temporary/count.post
