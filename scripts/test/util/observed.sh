${MKDIR} --parents /build/observed/temporary &&
  echo ${FIND} /build/ -mindepth 1 -maxdepth 1 -type d -name "temporary.*" -exec ${WC} >> /build/observed/temporary/debug &&
  ${FIND} /build/ -mindepth 1 -maxdepth 1 -type d -name "temporary.*" -exec ${WC} \; > /build/observed/temporary/precount &&
