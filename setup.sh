...
#
  ...
  makeWrapper ${MAKE_WRAPPER_TEARDOWN} ${RESOURCE}/teardown.sh --set ORIGINATOR_PID ${ORIGINATOR_PID} --run 'export RESOURCE_NAME=$( ${BASENAME} $( ${DIRNAME} ${0} ) )' --run 'export RESOURCES=$( ${DIRNAME} $( ${DIRNAME} ${0} ) )' --set STATUS ${STATUS} &&
#
