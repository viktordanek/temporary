                                                                ...
                                                                in
                                                                    ''
                                                                        ${ pkgs.coreutils }/bin/mkdir $out &&
                                                                            ...
                                                                            ${ pkgs.coreutils }/bin/ln --symbolic ${ pkgs.writeShellScript "observed-internal" ( if builtins.pathExists ( self + "/test/util/observed-internal.nix" ) then ( builtins.concatStringsSep " &&\n" [ ( builtins.import ( self + "/scripts/test/util/observed-internal.nix" ) resources "${ pkgs.coreutils }/bin/echo" ) "${ builtins.concatStringsSep "" [ "$" "{" "SLEEP" "}" ] } 10s" "${ pkgs.coreutils }/bin/echo TARGET_PID=${ builtins.concatStringsSep "" [ "$" "{" "$" "}" ] }" "${ builtins.concatStringsSep "" [ "$" "{" "FIND" "}" ] } /build -mindepth 2 -maxdepth 2 -type f -name temporary -exec ${ builtins.concatStringsSep "" [ "$" "{" "GREP" "}" ] } ^temporary {} \\; | ${ builtins.concatStringsSep "" [ "$" "{" "WC" "}" ] } --lines > /build/observed/temporary/counts/mid" ] ) else self + "/scripts/test/util/error.nix" ) } $out/bin/observed-internal.sh &&
