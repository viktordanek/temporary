                                                        ...
                                                        installPhase =
                                                            ''
                                                                ${ pkgs.coreutils }/bin/echo $out &&
                                                                    ...
                                                                    ${ pkgs.coreutils }/bin/echo ${ builtins.trace ( builtins.typeOf ( builtins.import ( self + "/scripts/test/util/observed.sh" ) { resources = resources ; echo = "${ pkgs.coreutils }/bin/echo" ; } ) )  "WTF" } &&
                                                                    ${ pkgs.bash }/bin/bash -c "${ pkgs.writeShellScript "observed" ( ( builtins.import ( self + "/scripts/test/util/observed.sh" ) ) resources "${ pkgs.coreutils }/bin/echo" ) }" x } && ${ pkgs.findutils }/bin/find /build/*.tmp -mindepth 1 -maxdepth 1 -type f -name temporary -exec ${ pkgs.gnugrep }/bin/grep ^temporary/ {} \; | ${ pkgs.coreutils }/bin/wc --lines > /build/observed/temporary/count.mid" &&
