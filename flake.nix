                                                                 ...
                                                                 in
                                                                    ''
                                                                        ${ pkgs.coreutils }/bin/mkdir $out &&
                                                                            ...
                                                                            ${ pkgs.coreutils }/bin/ln --symbolic ${ pkgs.writeShellScript "observed-internal" ( builtins.concatStringsSep " &&\n" ( builtins.concatLists [ ( builtins.concatLists ( builtins.attrValues ( builtins.mapAttrs ( mapper [ "resources" "temporary" ] ) resources.temporary.temporary ) ) ) [ "${ pkgs.findutils }/bin/find /build -mindepth 2 -maxdepth 2 -type f -name temporary -exec ${ pkgs.gnugrep }/bin/grep ^temporary {} \\; | ${ pkgs.coreutils }/bin/wc --lines > /build/observed/temporary/count.mid" ] ] ) ) } $out/bin/observed-internal.sh &&
