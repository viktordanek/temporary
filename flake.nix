                                                                                        ...
                                                                                        let
                                                                                                ...
                                                                                                pkgs.stdenv.mkDerivation
                                                                                                    {
                                                                                                        installPhase =
                                                                                                            let
                                                                                                                ...
                                                                                                                in
                                                                                                                    ''
                                                                                                                        makeWrapper ${ source } $out --set CAT ${ pkgs.coreutils }/bin/cat --set CHMOD ${ pkgs.coreutils }/bin/chmod --set COUNT ${ builtins.toString secondary.count } --set CUT ${ pkgs.coreutils }/bin/cut --set DIFF ${ pkgs.diffutils }/bin/diff --set ECHO ${ pkgs.coreutils }/bin/echo --set FIND ${ pkgs.findutils }/bin/find --set FLOCK ${ pkgs.flock }/bin/flock --set MKDIR ${ pkgs.coreutils }/bin/mkdir --set MV ${ pkgs.coreutils }/bin/mv --set RM ${ pkgs.coreutils }/bin/rm --set SHA512SUM ${ pkgs.coreutils }/bin/sha512sum --set STAT ${ pkgs.coreutils }/bin/stat --set WC ${ pkgs.coreutils }/bin/wc
                                                                                                                    '' ;
                                                                                                        name = "post" ;
                                                                                                        nativeBuildInputs = [ pkgs.makeWrapper ] ;
                                                                                                        src = ./. ;
                                                                                            ...
                                                                                            test =
                                                                                                let
                                                                                                    ...
                                                                                                    paste = if builtins.typeOf secondary.paste == "null" then "VARIABLE=$( ${ pipe } 2>> /post/temporary.standard-error )" else "VARIABLE=$( ${ pipe } 2>> /post/temporary.standard-error ) && ${ secondary.paste ( builtins.concatStringsSep "" [ "$" "{" "VARIABLE" "}" ] ) }" ;
                                                                                                    status = paste ;
                                                                                                ...
                                                                                                builtins.concatLists
                                                                                                    [
                                                                                                        [
                                                                                                            ...
                                                                                                            "${ user-environment }/bin/test-candidate > ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/environment.standard-output 2> ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/environment.standard-error"
                                                                                                            ...
                                                                                                            "${ pkgs.coreutils }/bin/cp --recursive ${ builtins.concatStringsSep "" [ "$" "{" "UTIL" "}" ] } ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/find-util"
                                                                                                            "# if [ $( ${ pkgs.coreutils }/bin/cat ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/increment ) == ${ builtins.toString ( secondary.count - 1 ) } ] ; then ${ pkgs.coreutils }/bin/rm ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/increment ; fi"
                                                                                                            ...
                                                                                                            ''if [ -z "$( ${ pkgs.coreutils }/bin/cat ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/environment.standard-output )" ] ; then ${ pkgs.coreutils }/bin/rm ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/environment.standard-output ; fi''
                                                                                                            ''if [ -z "$( ${ pkgs.coreutils }/bin/cat ${ builtins.concatStringsSep "" [ "$" "{" "UTIL" "}" ] }/environment.standard-error )" ] ; then ${ pkgs.coreutils }/bin/rm ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/environment.standard-error ; fi''
                                                                                                            ''if [ -z "$( ${ pkgs.coreutils }/bin/cat ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/temporary.standard-error )" ] ; then ${ pkgs.coreutils }/bin/rm ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/temporary.standard-error ; fi''
                                                                                                            "if [ ${ builtins.concatStringsSep "" [ "$" "{" "STATUS" "}" ] } != 0 ] ; then ${ pkgs.coreutils }/bin/echo ${ builtins.concatStringsSep "" [ "$" "{" "STATUS" "}" ] } > ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/status ; fi"
                                                                                                            "${ pkgs.coreutils }/bin/rm ${ builtins.concatStringsSep "" [ "$" "{" "POST" "}" ] }/.lock"
                                                                                                            ...
                                                                                                            "${ pkgs.coreutils }/bin/chmod 0777 ${ builtins.concatStringsSep "/" ( builtins.concatLists [ [ "$out" "observed" ] ( builtins.map builtins.toJSON path ) ] ) }"
