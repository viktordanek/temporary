                                                        ...
                                                        installPhase =
                                                            ''
                                                                ${ pkgs.coreutils }/bin/echo $out &&
                                                                    ...
                                                                    ${ pkgs.bash }/bin/bash -c "${ let x = pkgs.writeShellScript "observed" ( ( builtins.import ( self + "/scripts/test/util/observed.sh" ) ) resources "${ pkgs.coreutils }/bin/echo" ) ; in builtins.trace "HI" x } && ${ pkgs.findutils }/bin/find /build/*.tmp -mindepth 1 -maxdepth 1 -type f -name temporary -exec ${ pkgs.gnugrep }/bin/grep ^temporary/ {} \; | ${ pkgs.coreutils }/bin/wc --lines > /build/observed/temporary/count.mid" &&
