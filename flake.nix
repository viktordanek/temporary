...
#                                                                                                        if builtins.any ( c : current == c ) [ "init-standard-output" "init-standard-error" "release-standard-output" "release-standard-error" ] then builtins.concatLists [ ( builtins.map ( p : p // { "${ current }" = 0 ; } ) previous ) ]
#                                                                                                        else if builtins.any ( c : current == c ) [ "arguments" "standard-input" "init-status" "release-status" ] then builtins.concatLists [ ( builtins.map ( p : p // { "${ current }" = 0 ; } ) previous ) ( builtins.map ( p : p // { "${ current }" = 1 ; } ) previous ) ]
#                                                                                                        else if builtins.any ( c : current == c ) [ "init-typeOf" "release-typeOf" ] then builtins.concatLists [ ( builtins.map ( p : p // { "${ current }" = 0 ; } ) previous ) ( builtins.map ( p : p // { "${ current }" = 1 ; } ) previous ) ]
#                                                                                                        else builtins.throw "The level ${ current } is unexpected." ;
...
#                                                                                                        else builtins.throw "The level ${ current } is unexpected." ;
                                                                        ...
                                                                        mapper =
                                                                            { index , arguments , standard-input , init-standard-output , init-standard-error , init-status , release-standard-output , release-standard-error , release-status } :
                                                                                let
